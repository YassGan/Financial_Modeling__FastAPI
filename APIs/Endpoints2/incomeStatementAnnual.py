...
IncomeStatement_Annual = APIRouter()
...
async def CompanyAnnualIncomeStatementInfo(symbol):
    print("Company with symbol", symbol, "made IncomeStatement API call annual information")
...
@IncomeStatement_Annual.get('/getAnnualIncomeStatement_company/{Symbol}')
async def create_IncomeStatement_Annual_api(Symbol: str):
    result = await CompanyAnnualIncomeStatementInfo(Symbol)
    return result
...
# element
async def CompanyAnnualIncomeStatementComparisonInsertion(symbol):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(api_url) as response:
            ...
            existing_data = companies_Annual_IncomeStatementCollection.find_one({"symbol": data[0]['symbol']})
            ...
            if existing_data:
                ...
                if api_date > db_date:
                    ...
                    companies_Annual_IncomeStatementCollection.replace_one({"symbol": data[0]['symbol']}, data[0])
                    
                    data[0]['symbol'] = data[0]['symbol']+"_"+db_date.strftime("%Y-%m-%d")
                    data[0]['date'] = db_date.strftime("%Y-%m-%d")


                    companies_Annual_IncomeStatementCollection.insert_one(data[0])
...
@IncomeStatement_Annual.get('/getQuarterBalanceSheet_company_comparison_insertion/{Symbol}')
async def create_IncomeStatement_Annual_api(Symbol: str):
    result = await CompanyAnnualIncomeStatementComparisonInsertion(Symbol)
    return result


# API that launches the function CompanyBalanceSheetInfoComparisonInsertion and use it with many symbols
@IncomeStatement_Annual.get('/Insert_Annual_IncomeStatement_information_Comparison')
async def Insert_IncomeStatement_Annual_information():

    dataframe = ['AAPL', 'LMNR','SU','LRCX']

    awaitable_tasks = [CompanyAnnualIncomeStatementComparisonInsertion(symbol) for symbol in dataframe]
